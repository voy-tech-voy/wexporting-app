...
1. Comparison mode: Bar chart comparisons (e.g., codec efficiency)
2. Simple mode: Text-only informational messages (e.g., GPU required)
...
class InfoTooltip(QWidget):
    ...
    def _on_theme_changed(self, is_dark: bool):
        ...
        self._update_styles()
        # Rebuild current content to refresh colors
        if hasattr(self, '_current_data'):
            if hasattr(self, '_is_simple_mode') and self._is_simple_mode:
                self.set_simple_mode(self._current_data['title'], self._current_data['message'], self._current_data.get('icon', 'info'))
            else:
    ...
    def set_comparison_mode(self, data: dict):
        """
        Display comparison bars (e.g., codec efficiency).
        
        Args:
            data: Dict with 'title', 'desc', and 'items' keys
                  items is list of tuples: (name, value, color_key, note)
        """
    ...
    def set_simple_mode(self, title: str, message: str, icon: str = "info"):
        """
        Display simple text-only tooltip (e.g., "GPU Required").
        ...
        """
        self._current_data = {'title': title, 'message': message, 'icon': icon}
...
class TooltipHoverFilter(QObject):
    """
    Event filter to attach InfoTooltip to any widget.
    Shows tooltip on hover with delay and fade-in animation.
    """
    
    def __init__(self, target_widget, tooltip_data: dict, delay_ms=250, simple_mode=False):
        """
        Initialize the hover filter.
        
        Args:
            target_widget: Widget to attach tooltip to
            tooltip_data: Data dict for tooltip content
            delay_ms: Delay before showing tooltip (default: 250ms)
            simple_mode: If True, use set_simple_mode, else set_comparison_mode
        """
        super().__init__(target_widget)
        self.target = target_widget
        self.tooltip = InfoTooltip(None)
