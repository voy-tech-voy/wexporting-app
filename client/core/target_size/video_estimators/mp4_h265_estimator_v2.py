...
Optimizes H.265/HEVC/MP4 videos for target file size.
...
def optimize_video_params(file_path: str, target_size_bytes: int, allow_downscale: bool = False, **kwargs) -> Dict:
    """
    Optimize H.265/HEVC/MP4 video for target size.
    ...
    """
    print(f"[H265_v2] Optimizing for {target_size_bytes} bytes, downscale={allow_downscale}")
    
    meta = get_media_metadata(file_path)
    if meta['duration'] == 0: 
        return {'video_bitrate_kbps': 1000, 'audio_bitrate_kbps': 64, 'encoding_mode': '2-pass', 'codec': 'libx265'}
    ...
    codec = "libx265"
    efficiency = 1.5  # 50% more efficient than H.264
    
    # 3. Resolution Scaling Based on BPP
    curr_w = meta['width']
    if allow_downscale:
        target_bpp = 0.08 / efficiency  # Lower BPP threshold due to better compression
        while (vid_bps / (curr_w * (curr_w * meta['height'] / meta['width']) * meta['fps'])) < target_bpp and curr_w > 480:
            curr_w = int(curr_w * 0.85)
            curr_w -= (curr_w % 2)
    
    print(f"[H265_v2] Result: {int(vid_bps/1000)}kbps, {curr_w}x{int(meta['height'] * (curr_w / meta['width'])) & ~1}")
