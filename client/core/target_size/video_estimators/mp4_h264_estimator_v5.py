...
class Estimator(EstimatorProtocol):
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[H264_v5] Estimating for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        try:
            ...
            print(f"[H264_v5] Resolved encoder: {codec} ({encoder_type})")
        except Exception as e:
            print(f"[H264_v5] GPU detection failed, using libopenh264: {e}")
            codec = "libopenh264"
        ...
        if encoder_type != EncoderType.CPU:
             # Hardware encoders often need slightly more bitrate for same quality/efficiency
             # But for target size, we stick to the math.
             pass
        ...
        print(f"[H264_v5] Estimated: {int(vid_bps/1000)}kbps, {curr_w}x{final_h}")
