...
class Estimator(EstimatorProtocol):
    """
    MP4 H.264 2-pass CBR encoding estimator.
    
    Strategy:
    - Pass 1: Analyze video, generate statistics
    - Pass 2: Encode with calculated bitrate
    """
    ...
    def description(self) -> str:
        return "H.264 2-pass CBR encoding"
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[H264_v2] Estimating for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        print(f"[H264_v2] Estimated: {int(vid_bps/1000)}kbps, {curr_w}x{int(meta['height'] * (curr_w / meta['width'])) & ~1}")
    ...
    def execute(
        ...
        **options
        ...
        """
        Execute 2-pass CBR encoding for H.264.
        
        This is the complete encoding pipeline for v2:
        1. Estimate parameters
        2. Run pass 1 (analyze)
        3. Run pass 2 (encode)
        """
        def emit(msg: str):
            if status_callback:
        ...
        try:
            ...
            input_stream, video_stream = self._build_stream(
                input_path, resolution_scale, rotation
            )
            ...
            # Build fresh stream for pass 2
            input_stream2, video_stream2 = self._build_stream(
                input_path, resolution_scale, rotation
            )
