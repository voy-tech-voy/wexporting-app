"""
MP4 H.265 Video Estimator v6
Optimizes H.265/MP4 videos for target file size using adaptive encoding.
...
class Estimator(EstimatorProtocol):
    """
    MP4 H.265 Adaptive encoding estimator.
    """
    ...
    def description(self) -> str:
        return "H.265 Adaptive (Hardware 1-pass / Software 2-pass)"
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[H265_v6] Estimating for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        try:
            ...
            codec, encoder_type = detector.get_best_encoder('MP4 (H.265)', prefer_gpu=True)
            print(f"[H265_v6] Resolved encoder: {codec} ({encoder_type})")
        except Exception as e:
            print(f"[H265_v6] GPU detection failed, using libx265: {e}")
            codec = "libx265"
        ...
        # 2. H.265 Specific Settings
        efficiency = 1.5  # H.265 is 50% more efficient than H.264
        ...
        if encoder_type != EncoderType.CPU:
             ...
             print(f"[H265_v6] Applied hardware encoder safety margin: {int(vid_bps/1000)}kbps")
        ...
        if allow_downscale:
            ...
            aspect_ratio = meta['height'] / meta['width']
            
            while True:
                h_for_calc = int(curr_w * aspect_ratio) & ~1
                current_bpp = vid_bps / (curr_w * h_for_calc * meta['fps'])
                if current_bpp >= target_bpp or curr_w <= 480:
                    break
                curr_w = int(curr_w * 0.85) & ~1
        ...
        print(f"[H265_v6] Estimated: {int(vid_bps/1000)}kbps, {curr_w}x{final_h}")
    ...
    def execute(
        ...
        **options
        ...
        input_args = transform_filters.get('input_args', {}).copy()
        ...
        try:
            ...
            log_error(e, context="mp4_h265_estimator_v6 / execute")
            return False
    ...
    def _run_process(self, cmd, duration, emit, should_stop, update_progress, drain_pipe, is_pass1=False):
        ...
        def read_pipes():
            try:
                while True:
                    ...
                    if duration > 0:
                        time_match = time_pattern.search(chunk_str)
                        if time_match:
        ...
        stdout_thread = threading.Thread(target=read_pipes)
        ...
        if process.returncode != 0:
            ...
            from client.utils.error_reporter import log_error
            log_error(
                Exception(f"FFmpeg mp4_h265 failed (returncode={process.returncode})"),
                context="mp4_h265_estimator_v6",
