"""
MP4 H.265/HEVC Video Estimator v4
...
class Estimator(EstimatorProtocol):
    """
    MP4 H.265/HEVC 2-pass CBR encoding estimator with transform filter support.
    ...
    def description(self) -> str:
        return "H.265 2-pass CBR (transforms + interruptible)"
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[H265_v4] Estimating for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        codec = "libx265"
        efficiency = 1.5  # 50% better than H.264
        ...
        if allow_downscale:
            target_bpp = 0.08 / efficiency  # Target 0.053 BPP for H.265 (better compression)
        ...
        print(f"[H265_v4] Estimated: {int(vid_bps/1000)}kbps, {curr_w}x{final_h}")
    ...
    def execute(
        ...
        **options
        ...
        """Execute 2-pass CBR encoding for H.265 with transform filters."""
        import threading
        import subprocess
        ...
        elif resolution_scale < 1.0 and not target_dims: # Fallback for no transforms but auto-resize
            vf_filters.append(f"scale={params['resolution_w']}:{params['resolution_h']}")
