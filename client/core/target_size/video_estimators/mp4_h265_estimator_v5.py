"""
MP4 H.265/HEVC Video Estimator v5
Optimizes H.265/HEVC/MP4 videos for target file size using adaptive encoding.
- Hardware Encoders (NVENC/QSV/AMF): 1-pass VBR/CBR (stable)
- Software Encoders (CPU): 2-pass CBR (accurate)

Codec efficiency: 1.5x (baseline)
...
class Estimator(EstimatorProtocol):
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[H265_v5] Estimating for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        try:
            ...
            print(f"[H265_v5] Resolved encoder: {codec} ({encoder_type})")
        except Exception as e:
            print(f"[H265_v5] GPU detection failed, using libx265: {e}")
            codec = "libx265"
        ...
        print(f"[H265_v5] Estimated: {int(vid_bps/1000)}kbps, {curr_w}x{final_h}")
