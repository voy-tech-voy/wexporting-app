"""
WebP Image Estimator v7
Optimizes WebP images for target file size using quality parameter (0-100).

Changes from v6:
- Skip files already under target size (avoids generation loss from re-encoding)
- Quality ceiling of 85 to prevent needless inflation (diminishing returns above ~85)
"""
import os
import subprocess
import tempfile
...
class Estimator(EstimatorProtocol):
    """WebP quality binary search estimator v7 - skip + quality ceiling."""
    ...
    def description(self) -> str:
        return "WebP binary search with skip logic and quality ceiling"
    ...
    def estimate(
        ...
        **options
        ...
        print(f"[WebP_v7] Estimating for {target_size_bytes} bytes ({target_size_bytes/1024:.1f} KB), downscale={allow_downscale}")

        # --- Fix 1: Skip if input already fits under target ---
        input_size = os.path.getsize(input_path)
        if input_size <= target_size_bytes:
            print(f"[WebP_v7] Input ({input_size/1024:.1f} KB) already under target ({target_size_bytes/1024:.1f} KB) - skipping")
        ...
        for scale in scales:
            ...
            # --- Fix 2: Quality ceiling at 85 ---
            # Binary search for HIGHEST quality that fits under target,
            # capped at QUALITY_CEILING to avoid diminishing returns inflation.
            ...
            for iteration in range(10):
                ...
                try:
                    ...
                    if size is not None:
                        print(f"[WebP_v7] Scale {int(scale*100)}%, Q{mid}: {size/1024:.1f} KB (target: {target_size_bytes/1024:.1f} KB)")
                    ...
                    print(f"[WebP_v7] Encode test failed: {e}")
                    high = mid - 1
                finally:
            ...
            if scale_best_quality >= 75:
                print(f"[WebP_v7] Found high quality Q{scale_best_quality} at scale {int(scale*100)}%, stopping search")
                break

        if best_res is None:
            best_res = self._fallback_estimate(input_path, target_size_bytes, meta, allow_downscale)

        print(f"[WebP_v7] Final: Q{best_res['quality']}, scale {int(best_res['scale_factor']*100)}%, estimated {best_res['estimated_size']/1024:.1f} KB")
        return best_res
    ...
    def _fallback_estimate(self, input_path, target_size_bytes, meta, allow_downscale):
        ...
        print("[WebP_v7] Target unreachable with standard settings")

        if allow_downscale:
            # Try progressively smaller sizes with minimum quality
            print("[WebP_v7] Attempting emergency downscaling...")
            current_scale = 0.5
            ...
            for attempt in range(20):
                ...
                try:
                    ...
                    if size and size <= target_size_bytes:
                        print(f"[WebP_v7] Emergency scale found: {int(current_scale*100)}% = {size/1024:.1f} KB")
                        return {
                            'quality': 10,
        ...
        print("[WebP_v7] Using minimum quality fallback")
        tmp = get_temp_filename()
        floor_size = 0
        try:
    ...
    def execute(
        ...
        **options
        ...
        try:
            ...
            print(f"[WebP_v7 DEBUG] Command: {' '.join(cmd)}")
            ...
            if result.returncode != 0:
                ...
                print(f"[WebP_v7 FFMPEG ERROR] {stderr_output}")
            ...
            import traceback
            emit(f"Error: {str(e)}")
            print(f"[WebP_v7 ERROR] {traceback.format_exc()}")
            return False
