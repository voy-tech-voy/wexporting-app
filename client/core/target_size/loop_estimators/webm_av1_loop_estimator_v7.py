...
class Estimator(EstimatorProtocol):
    ...
    def execute(self, input_path: str, output_path: str, target_size_bytes: int, 
        ...
        def run_ffmpeg_process(description, stream_obj, total_duration=0, is_pass2=False):
            ...
            print(f"[AV1_LOOP_v7 DEBUG] Command: {cmd}")
            ...
            if process.returncode != 0:
                ...
                print(f"[AV1_LOOP_v7 ERROR] FFmpeg failed. Command: {cmd}\nError Output:\n{error_msg}")
                emit(f"FFmpeg Error: {error_msg[-300:]}")
                from client.utils.error_reporter import log_error
                log_error(
                    Exception(f"FFmpeg webm_av1_loop failed (returncode={process.returncode})"),
                    context="webm_av1_loop_estimator_v7",
