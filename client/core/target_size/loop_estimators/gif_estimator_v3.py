"""
GIF Estimator v3 - Self-Contained Class-Based Estimator
...
class Estimator(EstimatorProtocol):
    """
    GIF Estimator v3 - Self-Contained Estimator
    ...
    def estimate(self, input_path: str, target_size_bytes: int, **options) -> Dict:
        ...
        print(f"[ESTIMATOR] Using loop_estimators/gif_estimator_v3.py")
        print(f"[GIF_v3] Optimizing for {target_size_bytes} bytes, downscale={allow_downscale}")
        ...
        p = valid[best]
        print(f"[GIF_v3] Result: {p[0]}fps, {p[1]} colors, scale {int(p[3]*100)}%")
    ...
    def _run_ffmpeg_process(self, description: str, stream_obj, emit, should_stop) -> bool:
        ...
        print(f"[GIF_v3 DEBUG] Command: {' '.join(cmd)}")
        ...
        if process.returncode != 0:
            ...
            print(f"[GIF_v3 ERROR] FFmpeg failed. Command: {' '.join(cmd)}\nError Output:\n{error_msg}")
