...
CHANGELOG:
- v3: Processes FULL video for estimation (no sampling) to fix size accuracy issues.
"""
import os
import tempfile
import ffmpeg
...
def optimize_gif_params(file_path: str, target_size_bytes: int, allow_downscale: bool = False, **kwargs) -> Dict:
    ...
    Processes the FULL video to ensure accurate file size.
    ...
    while left <= right:
        ...
        try:
            ...
            # Build graph correctly - USE FULL VIDEO
            in_stream = ffmpeg.input(file_path)
            scaled = (
                in_stream
                .filter('fps', fps)
                .filter('scale', w, h)
